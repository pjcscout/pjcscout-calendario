// Procesa las actas de jornada 2 de DH7 (jornada3-dh7-actas.json) y genera
...
const JORNADA = 3
const actas = JSON.parse(readFileSync('jornada3-dh7-actas.json', 'utf8'))
...
for (const [codActa, textoBruto] of Object.entries(actas)) {
  ...
  // "Tarjetas"/"Cuerpo Técnico" a veces aparecen una vez por equipo justo
  // detrás de sus suplentes, y a veces agrupadas al final para los dos
  // equipos juntas. Usar esas etiquetas como límite de "suplentes de A" es
  // frágil: si están agrupadas al final, el rango de A se come también toda
  // la plantilla de B. idxTitularesB (ya calculado) es un límite fiable en
  // los dos formatos, así que se usa siempre como tope de A.
  ...
  const tarjetasA = idxTarjetasA !== -1 && idxTarjetasA < idxTitularesB ? extraerTarjetas(lineas, idxTarjetasA, idxTitularesB) : []
...
writeFileSync('scripts/scraper/salida-dh7-eventos-j3.json', JSON.stringify(resultadosDh7, null, 2))
writeFileSync('scripts/scraper/salida-dh7-convocatorias-j3.json', JSON.stringify(convocatoriasDh7, null, 2))
