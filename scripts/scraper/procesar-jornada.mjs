// Procesa jornada<N>-detalle.json (N = env JORNADA) y genera las entradas
// RESULTADOS de esa jornada con eventos reales (gol, tarjeta_amarilla,
// tarjeta_roja) + el portero titular de cada equipo, listas para fusionar
// en resultados.js con aplicar-jornada-resultados.mjs.
...
const { resultadoFinal } = JSON.parse(readFileSync(`jornada${JORNADA}-detalle.json`, 'utf8'))
...
writeFileSync(`scripts/scraper/salida-eventos-j${JORNADA}.json`, JSON.stringify(nuevasEntradas, null, 2))
writeFileSync(`scripts/scraper/salida-convocatorias-j${JORNADA}.json`, JSON.stringify(convocatoriasVolcado, null, 2))
