...
// equipos juntas (visto por primera vez en jornada 3). Usar esas etiquetas
// como límite de "suplentes del equipo local" es frágil: si están
// agrupadas al final, el rango de A se come también toda la plantilla de
// B (todos los goles se atribuían al local). idxTitularesB es un límite
// fiable en los dos formatos, así que se usa siempre como tope de A.
...
writeFileSync(`scripts/scraper/salida-dh7-eventos-j${JORNADA}.json`, JSON.stringify(resultadosDh7, null, 2))
writeFileSync(`scripts/scraper/salida-dh7-convocatorias-j${JORNADA}.json`, JSON.stringify(convocatoriasDh7, null, 2))
